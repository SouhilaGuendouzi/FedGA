...
from utils.create_datasets import get_FashionMNIST
from utils.create_datasets import get_MNIST
import numpy as np
                ...
                try:
                 ...
                 if (message.subject=="LocalModels"):
                  ...
                  threading.Thread(target= self.SearchANdRequestTlModel, args=(message.data,)).start()
                    ...
                    threading.Thread(target=  self.ElectAggregator, args=()).start()
      ...
      if (message !=''): #message=[idEdge,addressEdge,domain, task]
        ...
        while (i< len(self.registry) and stop==False):
          if (self.registry[i][4]==message[2] and self.registry[i][5] ==message[3]):  #same domain and task
            ...
            obj = [message[0],self.registry[i][0]] #IdEdge (that needs the model), IdEdge_cible( that contains the complete model)
          ...
          for fog in self.active_fogs:
              ...
              for usr in fog[3]:
                ...
                while (i< len(self.registry) and stop==False):
                  ...
                  if (usr[4]==self.registry[i][4] and usr[4]==self.registry[i][4] and usr[2]>self.registry[i][3] ): #same domain and task but high accuracy >>
...
#*****************************************************************************************# 
    ...
    def ElectAggregator(self):
       if (self.numberFogsreceived==len(self.active_fogs)):
        #print(f'My capacity: {self.capacity} and My priority: {self.priority}')
        capacity=self.capacity
        priority= self.priority
        self.aggregatorSocket=self.server
        idenAggregator= [self.HOST,self.PORT,-1]
        for fog in self.active_fogs:
          ...
          if ((capacity< fog[4]) or ( (capacity== fog[4]) and (priority< fog[5])) ):
            ...
            capacity=fog[4]
            priority=fog[5]
            idenAggregator= [fog[2][0],fog[6],fog[0]] # (address, portBakcup, id ) of the elected aggregator
        ...
        if (self.aggregatorSocket==self.server):
            ...
            self.FLrounds=self.args.epochs
            self.add_message("Cloud> Starting FL \n")
            self.send_messages_to_all(self.args.aggr,"FLstart") #broadcat to all fogs
        else :
          self.aggregator="Fog"
          self.add_message(f"Cloud> The elected aggregator is Fog {idenAggregator[2]}  \n")
          self.send_message_to_fog( self.aggregatorSocket,args.aggr,'ElectedAggregator')
          for user in self.active_fogs:
            if (user[1]!=self.aggregatorSocket):
             print(f'the elected node is :{idenAggregator} ')
             self.send_message_to_fog(user[1],idenAggregator,"Aggregator")  # send to all fogs the (address, port, id)  of the elected aggregator except the elected one
