...
class Edge(object):
     ...
     def connect(self):
       ...
       try:
           ...
           self.socket.connect((self.adrFog, self.portFog))
           print("Successfully connected to server")
           self.add_message(f'Edge{self.id}> Successfully connected to Fog server {self.adrFog}:{self.portFog} \n')
         
          
           self.send_message(self.id,'Connection')
           
       except:
        print("Unable to connect to server", f"Unable to connect to Fog server {self.adrFog}:{self.portFog}")
